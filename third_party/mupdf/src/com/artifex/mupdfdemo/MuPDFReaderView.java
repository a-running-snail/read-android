...
public class MuPDFReaderView extends ReaderView {
	...
	private int pageClickX;
	private int pageClickY;
	...
	public MuPDFReaderView(Activity act) {
		...
		pageClickX = dm.widthPixels/4;
		pageClickY = dm.heightPixels/4;
	}
	
	public void forcePortrait() {
		int min = Math.min(pageClickX, pageClickY);
		int max = Math.min(pageClickX, pageClickY);
		pageClickX = min;
		pageClickY = max;
	}
	...
	public boolean onSingleTapUp(MotionEvent e) {
		...
		if (!mSelecting && !tapDisabled) {
			...
			} else if (e.getX() < pageClickX) {
				super.smartMoveBackwards();
			} else if (e.getX() > super.getWidth() - pageClickX) {
				super.smartMoveForwards();
			} else if (e.getY() < pageClickY) {
				super.smartMoveBackwards();
			} else if (e.getY() > super.getHeight() - pageClickY) {
				super.smartMoveForwards();
			} else {
				onTapMainDocArea();
			}
