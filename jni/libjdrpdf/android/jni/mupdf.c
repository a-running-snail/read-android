...
#define LOG_TAG "JD_READER_PDF_SDK"
...
/* -- { */
static char * filter_key = NULL;
static int    filter_len = 0;

static char * filter_book = NULL;
static int    filter_blen = 0;
static char * filter_uuid = NULL;
static int    filter_ulen = 0;
static char * filter_rand = NULL;
static int    filter_rlen = 0;
static char * nsft_dir    = NULL;
...
char * JD_READER_GET_KEY (void)
{
    return filter_key;
}

int    JD_READER_GET_LEN (void)
{
    return filter_len;
}

char * JD_READER_GET_PATH (void)
{
    return nsft_dir;
}

struct HashSetHelper_s
{
    JNIEnv* jenv;
    jclass cls;
    jmethodID cid;
...
typedef struct HashSetHelper_s HashSetHelper;

int HashSetHelper_init(HashSetHelper* that, JNIEnv* env)
{
    that->jenv = env;
    that->cls = (*(that->jenv))->FindClass(that->jenv, "java/util/HashSet");
    if (that->cls)
    {
...
jobject HashSetHelper_create(HashSetHelper* that)
{
    return that->valid ? (*(that->jenv))->NewObject(that->jenv, that->cls, that->cid) : NULL;
}

void HashSetHelper_add(HashSetHelper* that, jobject hashSet, jobject obj)
{
    if (that->valid && hashSet)
    {
        (*(that->jenv))->CallBooleanMethod(that->jenv, hashSet, that->midAdd, obj);
    }
}
/* -- } */
...
JNI_FN(MuPDFCore_openFile)(JNIEnv * env, jobject thiz, jstring jfilename)
{
    ...
    {
        jclass jc   = (*env)->FindClass(env, "com/jingdong/app/reader/pdf/PDFDeviceInfo");
        jfieldID jf = (*env)->GetStaticFieldID(env, jc, "KeyBook", "[B");
        jbyteArray ja  = (*env)->GetStaticObjectField(env, jc, jf);
        filter_blen = (*env)->GetArrayLength(env, ja);
        
        if (filter_blen > 0)
        {
            unsigned char * temp = (*env)->GetByteArrayElements(env, ja, JNI_FALSE);
            filter_book = (unsigned char *) alloca (filter_blen + 1);
            memset(filter_book, 0, filter_blen + 1);
            memcpy(filter_book, temp, filter_blen);
            (*env)->ReleaseByteArrayElements(env, ja, temp, JNI_ABORT);
        }
    }
    {
        jclass jc   = (*env)->FindClass(env, "com/jingdong/app/reader/pdf/PDFDeviceInfo");
        jfieldID jf = (*env)->GetStaticFieldID(env, jc, "KeyUUID", "[B");
        jbyteArray ja  = (*env)->GetStaticObjectField(env, jc, jf);
        filter_ulen = (*env)->GetArrayLength(env, ja);

        if (filter_ulen > 0)
        {
            unsigned char * temp = (*env)->GetByteArrayElements(env, ja, JNI_FALSE);
            filter_uuid = (unsigned char *) alloca (filter_ulen + 1);
            memset(filter_uuid, 0, filter_ulen + 1);
            memcpy(filter_uuid, temp, filter_ulen);
            (*env)->ReleaseByteArrayElements(env, ja, temp, JNI_ABORT);
        }
    }
    {
        jclass jc   = (*env)->FindClass(env, "com/jingdong/app/reader/pdf/PDFDeviceInfo");
        jfieldID jf = (*env)->GetStaticFieldID(env, jc, "KeyRand", "[B");
        jbyteArray ja = (*env)->GetStaticObjectField(env, jc, jf);
        filter_rlen = (*env)->GetArrayLength(env, ja);
        
        if (filter_rlen > 0)
        {
            unsigned char * temp = (*env)->GetByteArrayElements(env, ja, JNI_FALSE);
            filter_rand = (unsigned char *) alloca (filter_rlen + 1);
            memset(filter_rand, 0, filter_rlen + 1);
            memcpy(filter_rand, temp, filter_rlen);
            (*env)->ReleaseByteArrayElements(env, ja, temp, JNI_ABORT);
        }
    }
    {
        jclass jc   = (*env)->FindClass(env, "com/jingdong/app/reader/pdf/PDFDeviceInfo");
        jfieldID jf = (*env)->GetStaticFieldID(env, jc, "NonStandardFontsPath", "Ljava/lang/String;");
        jstring ja = (*env)->GetStaticObjectField(env, jc, jf);
        
        nsft_dir = (*env)->GetStringUTFChars(env, ja, JNI_FALSE);
    }
    
    if (filter_blen != 0 && filter_ulen != 0 && filter_rlen != 0 &&
        filter_book != NULL && filter_uuid != NULL && filter_rand != NULL)
    {
        extern int GetContentKeyBuf(const char* pRightFileBuf, int nRightFileBuflen, char* pDevIDHashCipher, char* pRandomCipher, char** ppKey, int* nKeyLen);
        GetContentKeyBuf(filter_book, filter_blen, filter_uuid, filter_rand, &filter_key, &filter_len);
    }
    ...
    {
        pdf_document * xref = pdf_open_document_no_run(ctx, filename);
        int pagecount = pdf_count_pages(xref);
        ...
        HashSetHelper hsh;
        HashSetHelper_init(&hsh, env);
        jobject hashSet = HashSetHelper_create(&hsh);
        
        for(; i < pagecount; i++)
        {
            pageobj = xref->page_objs[i];
            resobj = pdf_dict_gets(pageobj, "Resources");
            font = pdf_dict_gets(resobj, "Font");
            ...
            if (font)
            {
                ...
                char *fontname;
                jstring jsFontname;
                len = pdf_dict_len(font);
                
                for(j = 0; j < len; j++)
                {
                    fontdict = pdf_dict_get_val(font, j);
                    
                    if (pdf_is_dict(fontdict))
                    {
                        type = pdf_dict_gets(fontdict, "Subtype");
                        typename = pdf_to_name(type);
                        
                        if (type && !strcmp(typename, "Type0"))
                        {
                            dfonts = pdf_dict_gets(fontdict, "DescendantFonts");
                            
                            if (dfonts)
                            {
                                dfont = pdf_array_get(dfonts, 0);
                                fontdes = pdf_dict_gets(dfont, "FontDescriptor");
                            }
                        }
                        else if (type && (!strcmp(typename, "Type1") || !strcmp(typename, "TrueType") || !strcmp(typename, "MMType1")))
                        {
                            fontdes = pdf_dict_gets(fontdict, "FontDescriptor");
                        }
                        else
                        {
                            continue;
                        ...
                        if(!pdf_is_null(fontdes) && (NULL != fontdes))
                        {
                            obj1 = pdf_dict_gets(fontdes, "FontFile");
                            obj2 = pdf_dict_gets(fontdes, "FontFile2");
                            obj3 = pdf_dict_gets(fontdes, "FontFile3");
                        ...
                        fontname = pdf_to_name(baseFont);
                        
                        if ((!strchr(fontname, ',') || strchr(fontname, '+')) && fontdes && !pdf_is_null(fontdes))
                        {
                            fontname = pdf_to_name(pdf_dict_gets(fontdes, "FontName"));
                        }
                        
                        if(strncmp(fontname, "EU", 2) == 0)
                        {
                            jstring jsFontname = (*env)->NewStringUTF(env, fontname);
                            HashSetHelper_add(&hsh, hashSet, jsFontname);
                            (*env)->DeleteLocalRef(env, jsFontname);
                        }
                    }
                }
            }
        ...
        {
            jclass jc   = (*env)->FindClass(env, "com/jingdong/app/reader/pdf/PDFDeviceInfo");
            jfieldID jf = (*env)->GetStaticFieldID(env, jc, "NonStandardFontsName", "Ljava/util/HashSet;");
            (*env)->SetStaticObjectField(env, jc, jf, hashSet);
        }
    }
