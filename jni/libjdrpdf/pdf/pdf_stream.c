...
build_filter(fz_stream *chain, pdf_document * xref, pdf_obj * f, pdf_obj * p, int num, int gen, fz_compression_params *params)
{
    ...
    else if (!strcmp(s, "JDPDFENCRYPTBY360BUY"))
    {
/*
        char jd_key[] = {0x31, 0x31, 0x37, 0x66, 0xd9, 0x98, 0x6b, 0x81,
            0xba, 0xe1, 0x80, 0x02, 0xd9, 0x27, 0x23, 0x6f,
            0x0f, 0xdb, 0xed, 0x11, 0xd3, 0x36, 0xc7, 0x77,
            0x1b, 0xa9, 0x4b, 0xcb, 0x0b, 0x00, 0xea, 0x77};
        int jd_len = 32;
*/
        extern char * JD_READER_GET_KEY (void);
        extern int    JD_READER_GET_LEN (void);
        
        return fz_open_jddrm(chain, JD_READER_GET_KEY(), JD_READER_GET_LEN());
    }
