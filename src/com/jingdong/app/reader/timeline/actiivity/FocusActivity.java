...
public class FocusActivity extends BaseActivityWithTopBar {
	...
	protected void onCreate(Bundle arg0) {
		...
		focusAdapter = new FocusAdapter(FocusActivity.this, usersLists);
		mListView.setAdapter(focusAdapter);
		// mListView.setEmptyView(empty_view);
		if (LoginUser.getpin().equals(jd_username)) {
			text.setText("您还没有关注任何人哦");
		} else {
			text.setText("他还没有关注任何人");
		}
	...
	protected void onResume() {
		...
		initField();
		getMyWant();
		StatService.onPageStart(this, MZBookApplication.getInstance().getString(R.string.mtj_my_guanzhu));
	}
	
	@Override
	protected void onPause() {
		super.onPause();
		StatService.onPageEnd(this, MZBookApplication.getInstance().getString(R.string.mtj_my_guanzhu));
	}
	...
	private void getMyWant() {
		WebRequestHelper.get(
				...
				RequestParamsPool.getFocusParams(user_id, currentSearchPage
						+ "", perSearchCount + ""),
				new MyAsyncHttpResponseHandler(FocusActivity.this) {
