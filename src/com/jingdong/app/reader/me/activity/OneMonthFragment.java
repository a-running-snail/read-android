...
//		orderLists.clear();
	...
	public View onCreateView(LayoutInflater inflater,
		...
		//Talking-Data
		TCAgent.onPageStart(getActivity(), "订单_近一个月");
	...
	public void onDestroyView() {
		super.onDestroyView();
		//Talking-Data
		TCAgent.onPageEnd(getActivity(), "订单_近一个月");
	}

	private void searchOrder() {

		WebRequestHelper.get(URLText.JD_BASE_URL, RequestParamsPool
				.getOrderParams(currentSearchPage + "", perSearchCount + ""),
	...
	private class OrderAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				...
				holder.sendbookTipImg = (ImageView) convertView
						.findViewById(R.id.sendbook_tip);
			...
			if(orderLists.get(position).getOrderMode() == 6){
				holder.sendbookTipImg.setVisibility(View.VISIBLE);
			}else
				holder.sendbookTipImg.setVisibility(View.GONE);
			...
			if (orderLists.get(position).getPrice() == 0) {
				...
				if (orderLists.get(position).getOrderMode() == 0||orderLists.get(position).getOrderMode() == 4 ||orderLists.get(position).getOrderMode() == 6) {
			...
			convertView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					intent.putExtra("order_id", orderLists.get(position).getOrderId()+"");
					intent.putExtra("order_money", orderLists.get(position).getPrice()+"");
					intent.putExtra("order_time", orderLists.get(position).getOrderTime());
					intent.putExtra("order_statue", orderLists.get(position).getOrderStatus()+ "");
					intent.putExtra("order_mode", orderLists.get(position).getOrderMode()+"");
	...
	public void setUserVisibleHint(boolean isVisibleToUser) {
		...
		if(isVisibleToUser){
			StatService.onPageStart(getActivity(), MZBookApplication.getInstance().getString(R.string.mtj_my_dingdan_month));
		}else{
			StatService.onPageEnd(getActivity(), MZBookApplication.getInstance().getString(R.string.mtj_my_dingdan_month));
		}
	}
