...
public class BookNoteListActivity extends BaseActivityWithTopBar {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		outlineList = getIntent().getParcelableArrayListExtra(BookNoteListActivity.TOCLabelListKey);
		ebookid = getIntent().getLongExtra(BookNoteListActivity.EBOOK_ID, 0);
		docid = getIntent().getIntExtra(BookNoteListActivity.DOCUMENT_ID, 0);
		userId = getIntent().getStringExtra(BookNoteListActivity.USER_ID);
		...
		this.setContentView(R.layout.activity_read_note);
		this.getTopBarView().setTopBarTheme(ThemeUtils.getTopbarTheme());
		this.getTopBarView().setTitle(getString(R.string.read_note));
		this.getTopBarView().setLeftMenuVisiable(false, 0);
		nullLayout = findViewById(R.id.readnote_null_layout);
		bottomContainer = findViewById(R.id.bottomContainer);
		bottomEditContainer = findViewById(R.id.bottomEditContainer);
		...
		bookNotesList = (ExpandableStickyListHeadersListView) findViewById(R.id.booknotesList);
		if (readNotes.size() == 0) {
		...
		bookNotesList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (isDeleteMode) {
					...
					TypedArray a = BookNoteListActivity.this.obtainStyledAttributes(new int[] {
				...
				data.putExtra(BookNoteListActivity.CHAPTER_ITEM_REF,
						readnote.spineIdRef);
				data.putExtra(BookNoteListActivity.PARA_INDEX,
						readnote.fromParaIndex);
				data.putExtra(BookNoteListActivity.OFFSET_IN_PARA,
						readnote.toOffsetInPara);
				BookNoteListActivity.this.setResult(
						BookPageViewActivity.RESULT_OK, data);
				BookNoteListActivity.this.finish();
				BookNoteListActivity.this.overridePendingTransition(
	...
	private void removeReadNotes(List<ReadNote> readNotes) {
		if (readNotes != null && readNotes.size() > 0) {
			...
			if (!LoginUser.getpin().equals(userId)) {
				int noteCount = listData.size();
				if (noteCount == 0) {
					MZBookDatabase.instance.deletImportNotes(LoginUser.getpin(),
							ebookid, docid, userId);
				}else {
					if (docid != 0) {
						MZBookDatabase.instance.updateNoteCount(
								LoginUser.getpin(), 0, docid,
								noteCount, userId);
					} else {
						MZBookDatabase.instance.updateNoteCount(
			...
			Intent intent = new Intent(AlreadyAddedNotesActivity.REMOVE_NOTES_ACTION_DONE);
	...
	class BookNotesAdapter extends BaseAdapter implements StickyListHeadersAdapter, SectionIndexer {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = LayoutInflater.from(BookNoteListActivity.this).inflate(
			...
			if (isDeleteMode) {
				TypedArray a = BookNoteListActivity.this.obtainStyledAttributes(
